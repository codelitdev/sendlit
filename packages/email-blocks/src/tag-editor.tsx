...
export function TagEditor({
  ...
  const canCreateNewOption =
    normalizedSearch.length > 0 && !existingValuesLower.has(normalizedSearch);
              ...
              {filteredOptions.map((option) => {
                ...
                return (
                  <CommandItem
                    key={option}
                    value={option}
                    aria-label={isSelected ? `${option}, added` : option}
                    onSelect={(currentValue) => toggleOption(currentValue)}
                  >
                    <div
