...
export async function updateContact(
  ...
  contactId: string,
  patch: Partial<
    Pick<Contact, "name" | "tags" | "active" | "subscribedToUpdates" | "customFields">
  >,
): Promise<Contact | null> {
  const [row] = await db
    .update(contacts)
    .set({ ...patch, updatedAt: new Date() })
    .where(and(eq(contacts.teamId, teamId), eq(contacts.contactId, contactId)))
    .returning();
  return row ?? null;
}
...
export async function getDeliveriesByContact(
  ...
  const rows = await db
    ...
    .innerJoin(sequences, eq(sequences.sequenceId, emailDeliveries.sequenceId))
    .where(
      and(
