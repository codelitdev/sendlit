...
// Authorization codes are short-lived (5 min) and only need to survive a single
// process; an in-memory map (same approach as MediaLit) is sufficient here.
...
const STATIC_CLIENTS: Record<string, { redirectUris: string[] }> = {
    "web-client": {
        redirectUris: [
            `${process.env.WEB_CLIENT || "http://localhost:3000"}/api/auth/callback/sendlit`,
...
export const oauthModel: OAuth2Server.AuthorizationCodeModel &
    ...
    async getClient(clientId: string, _clientSecret?: string) {
        const dyn = await findClientByClientId(clientId);
        if (dyn) {
            return {
                id: dyn.clientId,
    ...
    async saveToken(
        ...
        const accessToken = signAccessToken(accountId, clientId, scope);
        const refreshToken = signRefreshToken(accountId, clientId);
