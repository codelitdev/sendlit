...
oauthRouter.post(
    ...
    async (req: ExpressReq, res: ExpressRes) => {
        try {
            ...
            const pending = await findPendingAuthNotExpired(String(pendingId));
            if (!pending) {
                return res.json({
                    success: false,
            ...
            if (
                pending.otpSentAt &&
                Date.now() - pending.otpSentAt < OTP_RESEND_COOLDOWN_MS
            ) {
                return res.json({
                    success: false,
            ...
            await updatePendingAuth(String(pendingId), {
                email: emailValue,
                ...
                try {
                    ...
                    if (process.env.EMAIL_HOST && process.env.EMAIL_USER) {
                        ...
                        await transporter.sendMail({
                            ...
                            html: `<p>Enter this code to authorize SendLit access:</p><h2>${otp}</h2>`,
                        });
                    }
                } catch (mailErr: any) {
                    logger.error(
...
oauthRouter.post(
    ...
    async (req: ExpressReq, res: ExpressRes) => {
        try {
            ...
            const pending = await incrementOtpAttempts(pendingId);
            if (!pending) {
                return res.json({
                    success: false,
            ...
            await oauth.authorize(oauthReq, oauthRes, {
                authenticateHandler: {
                    handle: () => Promise.resolve({ id: accountId, email }),
                },
            });
