...
import { createApiKey, deleteApiKey, getApiKeysByTeamId } from "../../apikey/queries";
...
export function registerTeamTools(server: McpServer): void {
  server.registerTool(
    "list_teams",
    {
      description: "Returns all teams the authenticated account belongs to.",
    ...
    async (extra: any) => {
      ...
      try {
        ...
        return jsonResult({ items: teams.map((t) => ({ id: t.id, name: t.name })) });
      } catch {
  ...
  server.registerTool(
    "create_api_key",
    {
      description: "Creates a new API key for the current team. The full key value is only returned once — store it securely.",
      inputSchema: {
        name: z.string().min(1).describe("Human-readable label for this key"),
      },
    ...
    async (args: any, extra: any) => {
      ...
      try {
        ...
        return jsonResult({ key: key.key, name: key.name, teamId: key.teamId, createdAt: key.createdAt });
      } catch {
        return INTERNAL_ERROR;
      }
    },
  ...
  server.registerTool(
    "delete_api_key",
    {
      description: "Permanently deletes an API key. Any integrations using it will stop working immediately.",
      inputSchema: {
