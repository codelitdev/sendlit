...
export function registerSequenceTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async (args: any, extra: any) => {
      ...
      if (!teamId) return AUTH_ERROR;
      try {
        const sequence = await getSequenceBySequenceId(teamId, args.sequenceId);
  ...
  server.registerTool(
    ...
    async (args: any, extra: any) => {
      ...
      try {
        const sequence = await updateSequence({ teamId, sequenceId, ...patch });
        if (!sequence) return NOT_FOUND;
        return jsonResult(sequence);
      } catch (err: any) {
        return errorResult(err.message);
    ...
    "update_sequence_email",
    {
      ...
      inputSchema: {
        ...
        delayInMillis: z
          .number()
          .optional()
          .describe("Milliseconds after the previous email (sequences only)"),
  ...
  server.registerTool(
    "get_sequence_subscribers",
    {
      ...
      outputSchema: z.object({ items: z.array(z.string()), total: z.number() }),
      annotations: {
        readOnlyHint: true,
        idempotentHint: true,
        openWorldHint: false,
    ...
    async (args: any, extra: any) => {
      ...
      try {
        ...
        if (!sequence) return NOT_FOUND;
        const [items, total] = await Promise.all([
          getSubscribers({ sequenceId: args.sequenceId, page: args.page, limit: args.limit }),
          getSubscribersCount(args.sequenceId),
        ]);
        return jsonResult({ items, total });
      } catch {
